...
# Revised to a version adapted for the timestamp-local joint imputation framework.

import numpy as np
import torch
import torch.nn as nn

from diy_pypots.nn.modules.gpvae.layers import (
    GpvaeEncoder,
    rbf_kernel,
    diffusion_kernel,
    matern_kernel,
...
class BackboneGPVAE_GTEE(nn.Module):
    def __init__(
        self,
        input_dim,
        time_length,
    ...
    def impute(self, X, missing_mask, n_sampling_times=1):
        ...
        x_output = decode_x_mean.reshape(
            n_sampling_times, n_samples, n_steps, n_features
        ).permute(1, 0, 2, 3)
        return x_output  # n_samples, n_sampling_times, n_steps, n_features
    ...
    def forward(self, X, missing_mask):
        ...
        x_output = px_z.mean
        x_output = x_output.reshape(
            self.K * self.M, n_samples, n_steps, n_features
        ).permute(1, 0, 2, 3)  # n_samples, self.K * self.M, n_steps, n_features

        return -elbo, x_output
