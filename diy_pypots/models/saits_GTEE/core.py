"""
Description: The core wrapper assembles the submodules of SAITS-based GTEE imputation model.
...
from diy_pypots.nn.modules.saits.backbone import BackboneSAITS
...
class _SAITS_GTEE(nn.Module):
    ...
    def forward(
        ...
        fusion_in_feature_list = [TS_result, X_tilde_3, dist_logvars, missing_pattern_feature]
        ...
        X_tilde_final = torch.mul((1-fusion_weights),X_tilde_3) + torch.mul(fusion_weights,TS_result)
        ...
        results = {
            ...
            "imputed_data": imputed_data,  # batchsize × time_steps × feature_num
        ...
        if training:
            ...
            ORT_loss += self.customized_loss_func(X_tilde_final, X, missing_mask)
            ORT_loss /= 4
            ORT_loss = self.ORT_weight * ORT_loss
            
            MIT_loss = self.MIT_weight * self.customized_loss_func(
                X_tilde_final, X_ori, indicating_mask
            )
            
            main_loss = ORT_loss + MIT_loss
