"""
Preprocessing func for the dataset WindTurbine.

"""

# Created by Yuan Feng et al., 2026.
# Reference to Wenjie Du <wenjay.du@gmail.com>
# License: MIT License
...
def preprocess_wind_turbine_with_timestamps(
    file_path,
    rate,
    n_steps,
    pattern: str = "point",
    ...
    **kwargs,
    ...
    if has_nan:
        logger.info("Original data with NaN")
    else:
        logger.info("Original data without NaN")
    ...
    if with_time_stamps:
        time_stamps=pd.to_numeric(df["date"])
        
    unique_days = df["date"].dt.to_period("D").unique()
    selected_as_train = unique_days[:255]
    logger.info(f"days selected as train set are {selected_as_train[0]} - {selected_as_train[-1]}")
    selected_as_val = unique_days[255:292]
    logger.info(f"days selected as val set are {selected_as_val[0]} - {selected_as_val[-1]}")
    selected_as_test = unique_days[292:]
    logger.info(f"days selected as test set are {selected_as_test[0]} - {selected_as_test[-1]}")
    ...
    if with_time_stamps:
        train_timestamps_set = time_stamps[df["date"].dt.to_period("D").isin(selected_as_train)]
        val_timestamps_set = time_stamps[df["date"].dt.to_period("D").isin(selected_as_val)]
        test_timestamps_set = time_stamps[df["date"].dt.to_period("D").isin(selected_as_test)]
