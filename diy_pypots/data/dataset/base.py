...
# Supports timestamp data processing.
...
from diy_pypots.data.dataset.config import SUPPORTED_DATASET_FILE_FORMATS
from diy_pypots.data.utils import turn_data_into_specified_dtype
...
class BaseDataset_with_TS(Dataset):
    # Explicitly adds timestamp-related items over the original.
    def __init__(
        self,
        data: Union[dict, str],
        return_X_ori: bool,
        ...
        if isinstance(self.data, str):  # data from file
            ...
            if self.return_TS:
                assert (
                    "X_TS" in self.file_handle.keys()
                ), "The given dataset file doesn't contains completed TS information. Please double check."
            ...
            X_pred = None if "X_pred" not in data.keys() else data["X_pred"]
            y = None if "y" not in data.keys() else data["y"]
            X_TS = None if "X_TS" not in data.keys() else data["X_TS"]
            self.X, self.X_ori, self.X_pred, self.y, self.X_TS = self._check_array_input(
                X, X_ori, X_pred, y, X_TS, "tensor"
            )
            if self.return_X_ori:
                self.X, self.missing_mask = fill_and_get_mask_torch(self.X)
    ...
    def _fetch_data_from_file(self, idx: int) -> Iterable:
        """
        Function to get the specified sample set from the given file path, with added TS-related content.
        """
    ...
    def _check_array_input(
        ...
        X_pred: Optional[Union[np.ndarray, torch.Tensor]] = None,
        y: Optional[Union[np.ndarray, torch.Tensor]] = None,
        X_TS: Optional[Union[np.ndarray, torch.Tensor]] = None,
        out_dtype: str = "tensor",
    ) -> Tuple[
        Union[Tensor, ndarray],
        Optional[Union[Tensor, ndarray]],
        ...
        if X_TS is not None:
            X_TS = turn_data_into_specified_dtype(X_TS, out_dtype)
            X_TS = X_TS.to(torch.float32) if out_dtype == "tensor" else X_TS
            assert (X_TS.shape[0] == X_shape[0] and
                    X_TS.shape[1] == X_shape[1] and
                    X_TS.shape[2] == 1), f"The shape of X_TS is wrong, which is {X_TS.shape}."
            
        return X, X_ori, X_pred, y, X_TS
